...
for (i in 1:length(whales_p@polygons[[1]]@Polygons)){ # p = whales_p@polygons[[1]]@Polygons[1]
  ...
  num_vert = dim(whales_p@polygons[[1]]@Polygons[[i]]@coords)[1] - 1
...
write_file(sprintf('%d', length(whales_p@polygons[[1]]@Polygons)), poly, append=T)
for (i in 1:length(whales_p@polygons[[1]]@Polygons)){
  p = whales_p@polygons[[1]]@Polygons[[i]]
  hole_coord = spsample(p, 1, type = "random")@coords
  x = hole_coord[1, 1]
  y = hole_coord[1, 2]
...
```{r}
# david's little test area
  for (i in 1:length(whales_p@polygons[[1]]@Polygons)){
    p = whales_p@polygons[[1]]@Polygons[[i]]
    print(sprintf('# vertices: %d, area: %f', dim(p@coords)[1], p@area/1e6))
  }
```
