...
# start line with first node in path

# iterate through nodes in path
dist_tri = 0
prev_pt = as.numeric(path_tri$vpath[[1]][1])
ln_xy = data_frame(
  x=bc_poly_tri$P[prev_pt,1], 
  y=bc_poly_tri$P[prev_pt,2])
for (i in 2:length(path_tri$vpath[[1]])){ # i=2
  pt = as.numeric(path_tri$vpath[[1]][i])
...
path_tri_ln = SpatialLines(list(Lines(Line(ln_xy), ID="a")))
cat(sprintf('TOTAL length: %g\n', dist_tri))
```
...
coords = path_grid@lines[[1]]@Lines[[1]]@coords
dist_grid = 0
prev_pt = coords[1,]
...
# raster::drawExtent() xlim = c(-14257523, -14121332), ylim = c(6551470, 6647474)

plot(bc_poly_tri, xlim = c(-14257523, -14121332), ylim = c(6547005, 6647474))
plot(path_tri_ln, col='blue', lwd=5, add=T)
legend('topright', sprintf('n=%s\nd=%s', scales::comma(dim(bc_poly_tri$P)[1]), scales::comma(dist_tri)))
```
